...
def rr_temp_dir(tmpdir_factory):
    ...
    for file_data in file_datas:
        with open(
            os.path.join(temp_dir, "Data", file_data["file_name"]), "wb"
        ) as temp_file:
...
class TestDatValue:
    ...
    # Raw
    dv_raw = dat.DatValue(
        value=b"\x00\x00", offset=0, size=2, parent=None, specification=None
    )

    # Pointers
    dv_pointer = dat.DatValue(
    ...
    # Lists
    dv_list = dat.DatValue(
        value=(3, 8), offset=0, size=8, parent=None, specification=None
    )
    dv_list.children = [
    ...
    dv_list_reversed = dat.DatValue(
        value=(3, 8), offset=0, size=8, parent=None, specification=None
    )
    dv_list_reversed.children = [
        dat.DatValue(
            value=3, offset=8, size=4, parent=dv_list_reversed, specification=None
        ),
        dat.DatValue(
            value=2, offset=12, size=4, parent=dv_list_reversed, specification=None
        ),
        dat.DatValue(
            value=1, offset=16, size=4, parent=dv_list_reversed, specification=None
        ),
    ]
    ...
    def test_instance_pointer(self):
        ...
        assert (
            self.dv_pointer.data_end_offset
            == self.dv_pointer.value + self.dv_pointer.data_size
    ...
    def test_instance_list(self):
        ...
        # Property Tests
        assert (
            self.dv_list.data_size
            == self.dv_list.value[0] * self.dv_list.children[0].size
        ...
        assert (
            self.dv_list.data_end_offset
            == self.dv_list.value[1] + self.dv_list.data_size
        ), "Should be the pointer + sizeof(child)"
    ...
    def test_cmp(self, cmp_type, result, a, b):
        assert (
            getattr(a, cmp_type)(b) == result
        ), "%s for %s and %s should return %s" % (cmp_type, a, b, result)
...
class TestSpecificationErrors:
    ...
    def test_runtime_missing_specification(self, testspec_dat_file):
        ...
        assert (
            e.value.code == dat.SpecificationError.ERRORS.RUNTIME_MISSING_SPECIFICATION
        )
    ...
    def test_runtime_rowsize_mismatch(self, rr_temp_dir):
        ...
        with pytest.raises(dat.SpecificationError) as e:
            dr = df.read(
                ...
                specification=load(
                    os.path.join(spec_dir, "runtime_rowsize_mismatch.py")
                ),
            )
