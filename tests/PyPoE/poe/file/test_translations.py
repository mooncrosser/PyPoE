...
dbase_path = os.path.join(
    data_dir, "Metadata", "StatDescriptions", "descriptions_base.txt"
)
dextended_path = os.path.join(
    data_dir, "Metadata", "StatDescriptions", "descriptions_extended.txt"
)
...
def get_test(size, unid, nresults, values):
    ...
    results = [
        "tag_size%s_uq%s_v%s:%s" % (size, unid, i, " %s" * size)
...
class TestTranslationFileCache:
    ...
    def test_get_file(self, tcache, dbase, dextended):
        ...
        assert (
            tcache.get_file("Metadata/StatDescriptions/descriptions_extended.txt")
            == dextended
        ), "Files should be identical"
    ...
    def test_getitem(self, tcache, dbase, dextended):
        ...
        assert (
            tcache["descriptions_extended.txt"] == dextended
        ), "Files should be identical"
    ...
    def test_is_cache_working(self, tcache):
        ...
        assert (
            tcache["descriptions_extended.txt"] is a
...
class TestTranslationResults:
    ...
    @pytest.mark.parametrize(
        "tags,values,result,message,kwargs,rkwargs", functionality_tests
    )
