...
class SkillParserShared(parser.BaseParser):
    ...
    def _skill(self, ge, infobox, parsed_args, max_level=None, msg_name=None):
        ...
        if ae:
            try:
                tf = self.tc[
                    self.skill_stat_filter.skills[ae["Id"]].translation_file_path
                ]
        ...
        for data in level_data[1:]:
            ...
            for key in list(static["stats"]):
                ...
                if last["stats"][key]["values"] != data["stats"][key]["values"]:
        ...
        if key:
            infobox["projectile_speed"] = self.rr["Projectiles.dat"].index["Id"][
        ...
        for column, column_data in self._SKILL_COLUMN_MAP:
            ...
            if default is not None and gepl[0][column] == column_data["default"]:
                continue
